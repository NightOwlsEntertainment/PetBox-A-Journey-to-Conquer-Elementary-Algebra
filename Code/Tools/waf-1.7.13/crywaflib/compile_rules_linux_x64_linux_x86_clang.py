...
@conf
def check_linux_x64_linux_x86_clang_installed(conf):
	"""
...
@conf
def load_linux_x64_linux_x86_clang_common_settings(conf):
	"""
	Setup all compiler and linker settings shared over all linux_x64_linux_x86_clang configurations
	"""
	v = conf.env
...
@conf
def load_debug_linux_x64_linux_x86_clang_settings(conf):
	"""
	...
	conf.load_debug_linux_x86_settings()
	
@conf
def load_profile_linux_x64_linux_x86_clang_settings(conf):
	"""
	Setup all compiler and linker settings shared over all linux_x64_linux_x86_clang configurations for
