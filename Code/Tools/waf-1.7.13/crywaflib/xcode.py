...
# CryNotes: Add path for Info.pList
# CryNotes: Bundle must be in Bintemp/Code/GameSDK/MacLauncher...
# CryNotes: Dont' forget to check library/developer/.../mac_test../info.plist
...
class PBXProject(XCodeNode):
	...
	def add_task_gen(self, tg):
		if not getattr(tg, 'mac_app', False):
			self.targets.append(PBXLegacyTarget('build_darwin_profile', tg.name))
		else:
			target = PBXNativeTarget('build_darwin_profile', tg.name, tg.link_task.outputs[0].change_ext('.app'), tg.env)
