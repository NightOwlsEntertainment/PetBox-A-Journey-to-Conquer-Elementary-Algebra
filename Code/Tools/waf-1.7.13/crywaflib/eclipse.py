...
class eclipse(Build.BuildContext):
	...
	def impl_create_cproject(self, executable, waf, appname, workspace_includes, cpppath, source_dirs=[]):		
		...
		waf_build = '"%s" %s'%(waf, 'build_linux_profile --project-spec client')
		waf_clean = '"%s" clean_linux_profile  --project-spec client'%(waf)
		...
		addTargetWrap('install_linux_profile', False)
