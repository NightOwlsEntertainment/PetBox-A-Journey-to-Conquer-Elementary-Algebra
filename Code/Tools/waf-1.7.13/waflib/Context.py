...
class Context(ctx):
	...
	def execute_user_function(self, function, module):
		# Introduce global symbol
		def PathWrapper(path):
			return self.CreateRootRelativePath(path)
		
		def SettingsWrapper(*k, **kw):
			return self.SettingsWrapper_Impl(*k, **kw)
			
		module.Path 		= PathWrapper							
		module.Settings = SettingsWrapper
