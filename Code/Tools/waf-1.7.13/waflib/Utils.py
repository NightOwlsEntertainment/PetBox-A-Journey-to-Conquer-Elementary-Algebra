...
def h_file(fname, tg):
	...
	try:
		...
		# Very hacky way to check for a Q_OBJECT string, but this is the only
		# place without a server performance impact (here it inceases the build time by 0.1 s,
		# but even if the same file is read right after, the build time increases by 3 s
		if tg.bld and 'qt' in tg.features and 'Q_OBJECT' in content:
			if not hasattr(tg.bld, 'need_moccing_files'):
				tg.bld.need_moccing_files = set()
			tg.bld.need_moccing_files.add(fname.lower())
		m.update(content)
	finally:
		f.close()
